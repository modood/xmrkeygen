...
// SpendKey encodes spend key in hex format.
func (k *Key) SpendKey() string {
	return hex.EncodeToString(k.spendKey[:])
}

// ViewKey encodes view key in hex format.
func (k *Key) ViewKey() string {
	return hex.EncodeToString(k.viewKey[:])
}
...
func main() {
	key := NewKey()
	fmt.Println()
	fmt.Printf("  %s %s\n", "Mnemonic Seed:     ", strings.Join(key.Mnemonic(), " "))
	fmt.Printf("  %s %s\n", "Private Spend Key: ", key.SpendKey())
	fmt.Printf("  %s %s\n", "Private View Key:  ", key.ViewKey())
